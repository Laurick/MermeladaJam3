...
func get_tutorial_text() -> String:
	...
	if Global.gender == 0:
		...
		text += "¡Bienvenide"
	text += "  a tu nueva aventura en Piedras Salubres SA!\n\nComo ya te habrán informado en el curso de formación, tu deber como dependiente es ayudar a los clientes con sus necesidades más importantes. ¡Del tipo que sean! Un buen profesional siempre sabrá cuál es el producto que más le conviene a su cliente.\n\nPara ello, lo único que tendrás que hacer es elegir las runas y la piedra que mejor se ajusten a las necesidades del cliente. Fácil, ¿verdad? Desde Piedras Salubres SA nos comprometemos a facilitar todo el trabajo a nuestros empleados, de manera que,junto a este manual, habrá un vademécum en el que podrás consultar cualquier duda que tengas con respecto a las piedras y a las runas. ¡Esto está chupado\n\nLa satisfacción del cliente es nuestro beneficio y, para asegurarnos de ello, cada cierto tiempo irá a visitar la tienda un representante de ventas para evaluarte. Si la opinión de la tienda es negativa, recibirás menos cargamentos de piedras y runas. ¡Pero no temas! Eso no complicará tu función aquí.\n\nY, ahora, ¡ponte detrás del mostrador y ayuda a cuantos más clientes puedas!\n\nAtentamente, el equipo comercial de Piedras Salubres SA."
	return text
